...
public class LoginVerificationEmailService {

    private final MailSenderService mailSenderService;

    @Async
    public void sendLoginVerificationEmail(AppUser user, HttpServletRequest request) {
        try {
            ...
            if (userEmail == null || userEmail.isEmpty()) {
                log.warn("Cannot send login verification email: user email is empty for user ID: {}", user.getId());
                return;
            }

            Map<String, Object> variables = buildEmailVariables(user, request, userName, userEmail);

            EmailData emailData = EmailData.builder()
                    .to(userEmail)
                    .template(EmailTemplate.LOGIN_VERIFICATION)
                    .locale(userLocale)
                    .variables(variables)
                    .build();
            ...
            log.info("Login verification email sent successfully to user: {} ({})", userName, userEmail);
            
        } catch (Exception e) {
            log.error("Failed to send login verification email to user: {}", user.getId(), e);
        }
    }

    private Map<String, Object> buildEmailVariables(AppUser user, HttpServletRequest request, String userName, String userEmail) {
        ...
        variables.put("location", getLocationFromRequest(request));
    ...
    private String getLocationFromRequest(HttpServletRequest request) {
        // For now, return "Unknown" - you can integrate with a geolocation service later
        // if needed, using the IP address to determine location
        return "Unknown";
    }
}